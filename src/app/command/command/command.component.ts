...
export class CommandComponent {
  ...
  constructor(private builder: FormBuilder, private service: CommandService, private router: Router,/* , private alert: ToastrService,*/
  ...
  saveproducts1(){
let t=[]
t=this.invoicedetail.value
    console.log(this.invoicedetail.value)
    t.forEach((item:any)=>{
      
    this.service.saveproduct(item).subscribe(res => {
      console.log(res)    })
  })
  }
  ...
  customerchange() {
    ...
    let customercode =selectedValue;
    console.log('customercode from backside',customercode)
    this.service.GetCustomerbycode(customercode).subscribe(res => {
      let custdata: any;
      custdata = res;
      console.log("this is what you gonna get :",custdata)
      if (custdata != null) {
        this.invoiceform.get("deliveryAddress")?.setValue(custdata.name + ',' + custdata.address + ',' + custdata.phone + ',' + custdata.email);
  ...
  productchange(index: any) {
    const selectElement = document.getElementById("mySelect2") as HTMLSelectElement;
      const selectedValue = selectElement.value;
    console.log(selectedValue)
    this.invoicedetail = this.invoiceform.get("details") as FormArray;
    this.invoiceproduct = this.invoicedetail.at(index) as FormGroup;
    let productcode = selectedValue;
    this.service.GetProductbycode(productcode).subscribe(res => {
      ...
      if (proddata != null) {
        ...
        this.invoiceproduct.get("descripition")?.setValue(proddata.price);
